 ...
 * BESCHRIJVING
 * ==========================================================================
 * Dit bestand toont de contactpagina van de GamePlan Scheduler applicatie.
 * Het is een combinatie van PHP (server-side logica) en HTML (pagina-structuur).
 * De contactpagina biedt gebruikers verschillende manieren om hulp te krijgen:
 *
 * - Direct e-mailcontact met de ontwikkelaar
 * - Informatie over wie de applicatie heeft gemaakt
 * - Link naar de broncode op GitHub
 * - Veelgestelde vragen (FAQ) voor snelle zelfhulp
 *
 * WAAROM DEZE PAGINA?
 ...
 * Dit is ook een vereiste voor veel webstandaarden en richtlijnen.
 *
 * Gebruikersverhaal: "Als gebruiker wil ik contact kunnen opnemen met de
 * ontwikkelaar, zodat ik hulp kan krijgen bij problemen of vragen kan stellen."
 *
 * ==========================================================================
 * HOE DEZE PAGINA WERKT (VERZOEK-STROOM / REQUEST FLOW)
 * ==========================================================================
 * Deze pagina is EENVOUDIGER dan de formulierpagina's (add_event, add_schedule,
 * add_friend) omdat er GEEN formulierverwerking plaatsvindt.
 *
 * ┌─────────────────────────────────────────────────────────────────────┐
 * │ 1. Gebruiker klikt op "Contact" in de navigatiebalk of footer      │
 * │ 2. Browser stuurt GET-verzoek naar contact.php                     │
 * │ 3. PHP laadt functions.php en controleert sessie-timeout           │
 * │ 4. GEEN inlog-check: contactpagina is ook voor uitgelogde users    │
 * │ 5. HTML wordt naar de browser gestuurd en weergegeven              │
 * └─────────────────────────────────────────────────────────────────────┘
 *
 * OPMERKING: In tegenstelling tot add_event.php, add_schedule.php en
 * add_friend.php doet deze pagina GEEN isLoggedIn() controle.
 * De contactinformatie is voor iedereen toegankelijk, ook als je niet
 * bent ingelogd. Dit is een bewuste keuze: gebruikers die niet kunnen
 * inloggen moeten toch contact kunnen opnemen voor hulp.
 *
 * ==========================================================================
 * BEVEILIGING (Security)
 * ==========================================================================
 * 1. SESSIE-TIMEOUT: checkSessionTimeout() beëindigt inactieve sessies
 *    (als de gebruiker is ingelogd). Dit is de ENIGE beveiligingscontrole.
 *
 * 2. GEEN INLOG-VEREISTE: Bewuste keuze - contactinfo is publiek.
 *    Er wordt geen gevoelige data getoond, alleen contactgegevens.
 *
 * 3. GEEN FORMULIER: Er is geen invoerverwerking, dus geen risico op
 *    SQL-injectie of XSS via dit bestand. De pagina is "read-only".
 *
 * 4. EXTERNE LINKS: target="_blank" wordt gebruikt voor de GitHub-link.
 *    Dit opent de link in een nieuw tabblad zodat de gebruiker niet
 *    per ongeluk de applicatie verlaat.
 *
 * ==========================================================================
 * PAGINA-INHOUD (5 secties)
 * ==========================================================================
 * De pagina bevat 5 secties, allemaal binnen één Bootstrap card:
 *
 * ┌──────┬─────────────────────────┬──────────────────────────────────┐
 * │ Nr   │ Sectie                  │ Inhoud                           │
 * ├──────┼─────────────────────────┼──────────────────────────────────┤
 * │ 1    │ E-mail                  │ Klikbaar mailto: e-mailadres     │
 * │ 2    │ Ontwikkelaar            │ Naam, studentnr, opleiding, proj │
 * │ 3    │ GitHub Repository       │ Link naar broncode (target=blank)│
 * │ 4    │ Snelle Hulp             │ FAQ: inlog, wachtwoord, bugs     │
 * │ 5    │ Navigatie               │ "Terug naar Dashboard" knop      │
 * └──────┴─────────────────────────┴──────────────────────────────────┘
 *
 * ==========================================================================
 * VERSCHIL MET ANDERE PAGINA'S
 * ==========================================================================
 * contact.php is de EENVOUDIGSTE pagina van de applicatie:
 *
 * ┌────────────────────────┬──────────┬──────────┬──────────┬──────────┐
 * │ Eigenschap             │ contact  │ add_event│ add_sched│ add_frnd │
 * ├────────────────────────┼──────────┼──────────┼──────────┼──────────┤
 * │ Inlog vereist?         │ Nee      │ Ja       │ Ja       │ Ja       │
 * │ Heeft formulier?       │ Nee      │ Ja       │ Ja       │ Ja       │
 * │ POST-verwerking?       │ Nee      │ Ja       │ Ja       │ Ja       │
 * │ Database interactie?   │ Nee      │ Ja       │ Ja       │ Ja       │
 * │ JavaScript validatie?  │ Nee      │ Ja       │ Ja       │ Nee      │
 * │ Redirect na succes?    │ Nee      │ Ja       │ Ja       │ Ja       │
 * │ Alleen-lezen pagina?   │ Ja       │ Nee      │ Nee      │ Nee      │
 * └────────────────────────┴──────────┴──────────┴──────────┴──────────┘
 *
 * ==========================================================================
 * BESTANDSSTRUCTUUR
 * ==========================================================================
 * PHP-GEDEELTE (Server-side, minimaal):
 *   - functions.php inladen (require_once)
 *   - Sessie-timeout controleren (checkSessionTimeout)
 *   - GEEN inlog-check, GEEN formulierverwerking
 *
 * HTML-GEDEELTE (Client-side):
 ...
 *   - Main container met één grote Bootstrap card
 *   - 5 secties: e-mail, ontwikkelaar, GitHub, FAQ, navigatie
 ...
 * GEBRUIKTE BESTANDEN
 * ==========================================================================
 * - functions.php  : PHP helperfuncties (alleen checkSessionTimeout hier)
 ...
 * - script.js      : JavaScript functies (initialisatie, geen validatie hier)
 ...
 * - include       : Bestand laden voor header/footer (waarschuwing als niet gevonden)
 * - Geen $_POST, $_GET, $_SESSION directe toegang (alleen via functies)
 *
 * ==========================================================================
 * HTML CONCEPTEN GEBRUIKT IN DIT BESTAND
 * ==========================================================================
 * - Semantische elementen: <main>, <body>, <head>, <html>
 * - Kopteksten: <h1> (paginatitel), <h3> (sectietitels)
 * - Tekstelementen: <p> (paragraaf), <ul>/<li> (lijsten), <strong> (vetgedrukt)
 * - Links: <a href="mailto:"> (e-mail), <a target="_blank"> (extern)
 * - Bootstrap: card, card-body, container, btn, btn-primary, text-info
 * - Bootstrap spacing: mt-3, mt-4, mt-5, pt-5, mb-4
 * - Bootstrap thema: bg-dark, text-light (donker gaming thema)
 * ==========================================================================
 */
